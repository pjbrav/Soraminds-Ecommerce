...
Clone and run the full headless demo in one line:

```bash
# Linux / macOS
git clone https://github.com/pjbrav/Soraminds-Ecommerce.git && cd Soraminds-Ecommerce && python -m venv .venv && source .venv/bin/activate && pip install -r requirements.txt && python scripts/demo_run.py
```

```powershell
# Windows (PowerShell)
git clone https://github.com/pjbrav/Soraminds-Ecommerce.git; cd Soraminds-Ecommerce; python -m venv .venv; .\.venv\Scripts\Activate.ps1; pip install -r requirements.txt; python scripts\demo_run.py
```

Or step by step:

```bash
# 0. Clone (skip if you already have the folder)
git clone https://github.com/pjbrav/Soraminds-Ecommerce.git
cd Soraminds-Ecommerce
...
# 2. (Optional) Regenerate the demo data — the repo already ships these files
...
### Demo assets — where everything lives

| Path | What it is |
|---|---|
| `demo/SpiceHub_Kitchen_Menu_Upload_Template.xlsx` | The blank owner-facing template (with data-validation dropdowns) |
| `demo/messy_menu.xlsx` | Messy upload — 28 items with every demo issue baked in (missing price, bad category, duplicate, orphan…) |
| `demo/clean_menu.xlsx` | Clean upload — 27 items, publishes with zero fixes |
| `demo/photos.zip` | Photo set for the **messy** upload (28 real dish photos, incl. the orphan `chef_special.jpg` and the two whose price tags conflict with the sheet) |
| `demo/photos_clean.zip` | Photo set for the **clean** upload (27 photos, all matched, no orphans) |
| `demo/photos/`, `demo/photos_clean/` | The same photos, unzipped, for browsing |
| `demo/photo_credits.txt` | Sources and licensing for the demo dish photos (Wikimedia Commons) |
| `demo/output/kiosk_menu_v1_messy_fixed.json` | Published **menu-v1** — the messy path after the owner resolved everything |
| `demo/output/kiosk_menu_v2_clean.json` | Published **menu-v2** — the clean happy path |
| `demo/output/demo_run_log.txt` | Full transcript of a demo run — read this to see the pipeline working end to end without running anything |

Run `python scripts/seed_demo.py` to regenerate the spreadsheets and photo sets
from scratch (this is what produced them; the committed copies are identical in
structure, with realistic dish photos composited with their menu prices).
